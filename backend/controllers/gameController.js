...
export const joinGuest = async (req, res) => {
    try {
        ...
        if (room.host_id !== req.user.id) {
             return res.status(403).json({ message: 'Only host can add guests' });
        }
...
export const startGame = async (req, res) => {
  try {
    ...
    for (const player of players) {
      ...
      // Reset player states for new round if needed
...
export const bet = async (req, res) => {
  try {
    ...
    if (amount < room.current_highest_bet) {
      return res.status(400).json({ message: 'Bet must be >= current highest bet' });
    }
...
export const fold = async (req, res) => {
  try {
    ...
    const activePlayers = players.filter(p => p.is_active && p.id !== currentPlayer.id);
...
export const show = async (req, res) => {
    // Basic logic: a show implies ending the round and determining a winner manually or via cards.
    // For ledger app, we just mark that a show happened. The host then selects who won.
    try {
        const { room_id } = req.body;
        const room = await Room.findByPk(room_id);
        const io = req.app.get('io');
        ...
        res.json({ message: 'Show requested' });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server error' });
    }
...
export const pass = async (req, res) => {
  ...
  try {
    ...
    const room = await Room.findByPk(room_id);
    if (room.host_id !== req.user.id) {
       return res.status(403).json({ message: 'Only host can trigger pass' });
    }

    const players = await Player.findAll({ where: { room_id }, order: [['turn_order', 'ASC']] });
    
    await Transaction.create({
        user_id: player_id || req.user.id, // simplified
        room_id,
        amount: 0,
        type: 'pass'
    });
...
export const manualBet = async (req, res) => {
  ...
  try {
    ...
    const room = await Room.findByPk(room_id);
    if (room.host_id !== req.user.id) {
       return res.status(403).json({ message: 'Only host can add manual bets' });
    }
    const players = await Player.findAll({ where: { room_id }, order: [['turn_order', 'ASC']] });
...
export const declareWinner = async (req, res) => {
  try {
    ...
    const room = await Room.findByPk(room_id);
    if (room.host_id !== req.user.id) {
      return res.status(403).json({ message: 'Only host can declare winner' });
    }
    ...
    try {
        const room = await Room.findByPk(req.params.id, {
            include: [
                ...
                { model: Transaction, include: [{ model: User, attributes: ['username'] }], order: [['createdAt', 'DESC']] }
            ]
        });
